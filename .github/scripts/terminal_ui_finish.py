"""Fix measured window/contrast defects and superseded visual contracts.
Behavioral and safety tests are neither removed nor disabled.
"""
...
def edit(path, callback):
    p=root/path; old=p.read_text(); new=callback(old)
    if new == old: raise RuntimeError(f'No change: {path}')
    p.write_text(new)

def rep(s,a,b,n=1):
    if s.count(a)!=n: raise RuntimeError(f'Unexpected preimage: {a[:90]!r}: {s.count(a)}')
...
# Indexed(61) on Indexed(254) measured 4.41. Preserve the 4.5 contrast gate.
...
}.items(): edit(T+path,lambda s,mapping=mapping:test_text(s,mapping))

def chrome(s):
    s=rep(s,'vec![paper_label("davinci", th, true)]','vec![paper_label("DaVinci", th, true)]')
    return test_text(s,{'REVIEW CHANGES':'Review changes','SELECT A MODEL':'Select a model',
        'DAVINCI':'DaVinci','"━╸┄╺".contains(ch)':"ch == '─'",'row.contains("26 ·")':'row.contains("26 commands")',
        '            "LATEST",\n':'','" DaVinci  · AGENT"':'"DaVinci · AGENT"'})
...
edit('crates/davinci-tui/src/themes.rs',lambda s:test_text(s,{
    '38;2;243;217;13':'38;2;177;185;249','38;2;216;166;135':'38;2;217;155;130',
    '38;2;197;149;116':'38;2;170;170;170','48;2;94;28;22':'48;2;43;43;43',
