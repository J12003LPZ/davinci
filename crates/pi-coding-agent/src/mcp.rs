...
use pi_mcp::ConfigFile;

pub fn load(agent_dir: &Path, cwd: &Path, trusted: bool) -> ConfigFile {
    if let Ok(path) = std::env::var("PI_MCP_CONFIG") {
        return pi_mcp::load_path(Path::new(&path)).unwrap_or_default();
    }
    let user = pi_mcp::load_path(&agent_dir.join("mcp.json")).unwrap_or_default();
    if !trusted {
        return user;
    }
    let project = pi_mcp::load_path(&cwd.join(".pi").join("mcp.json")).unwrap_or_default();
    pi_mcp::merge(user, project)
}
