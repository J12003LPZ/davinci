...
use crate::{
    provision_message, EntryQuery, QueryOrder, SessionCreateOptions, SessionRepository,
};
...
pub fn run_conformance(repo: &mut dyn SessionRepository) -> ConformanceReport {
    ...
    check(&mut report, "entries and lanes", || {
        let mut session = repo.create(SessionCreateOptions {
            cwd: "/conformance".into(),
            id: Some("entries".into()),
            ..SessionCreateOptions::default()
        }).map_err(|error| error.to_string())?;
        let first = session.append_entry(provision_message("first"), "main").map_err(err)?;
        let second = session.append_entry(provision_message("second"), "main").map_err(err)?;
        if second.parent_id() != Some(first.id()) {
            return Err("lane parent was not assigned from the previous leaf".into());
        }
        let oldest = session.find_entries(EntryQuery {
    ...
    check(&mut report, "queries and facts", || {
        ...
        session.append_entry(provision_message("named"), "main").map_err(err)?;
    ...
    check(&mut report, "repository create list open delete", || {
        let created = repo.create(SessionCreateOptions {
            cwd: "/conformance".into(),
        ...
        opened.append_entry(provision_message("reopen"), "main").map_err(err)?;
    ...
    check(&mut report, "validation rejects duplicate ids", || {
        ...
        let first = session.append_entry(provision_message("once"), "main").map_err(err)?;
