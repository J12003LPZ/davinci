...
fn rebuild_ctrl_u_belongs_to_the_editor() {
    ...
    let flow = app::handle_key(&mut m, KeyEvent::new(KeyCode::Char('u'), KeyModifiers::CONTROL));
...
fn rebuild_settings_search_selects_the_source_row_and_keeps_chat_draft() {
    let mut m = model();
    m.settings_rows = vec![
        SettingRow { key: "autocompact".into(), label: "Auto-compact".into(), ..Default::default() },
        SettingRow { key: "transport".into(), label: "Transport".into(), values: vec!["auto".into(), "sse".into()], ..Default::default() },
    ];
    m.screen = Screen::Settings;
    ...
    let rows = views::settings::lines(&m).iter().map(ToString::to_string).collect::<Vec<_>>().join("\n");
    assert!(!rows.contains("Auto-compact"));
    assert_eq!(app::handle_key(&mut m, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)), app::Flow::Choose(Choice::Setting(1)));
}
...
fn rebuild_model_search_does_not_reindex_equal_model_names() {
    let mut m = model();
    m.catalog = vec![
        CatalogRow { provider: "provider-a".into(), id: "model".into(), name: "Same name".into(), ..Default::default() },
        CatalogRow { provider: "provider-b".into(), id: "model".into(), name: "Same name".into(), ..Default::default() },
    ];
    m.screen = Screen::Models;
    type_text(&mut m, "provider-b");
    assert_eq!(m.catalog.len(), 2, "filtering must not rewrite the host collection");
    assert_eq!(m.catalog_index, 1);
    assert_eq!(app::handle_key(&mut m, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)), app::Flow::Choose(Choice::Catalog(1)));
}

#[test]
fn rebuild_zero_search_results_cannot_accept_a_hidden_model() {
    let mut m = model();
    m.catalog = vec![CatalogRow { name: "Model".into(), ..Default::default() }];
    m.screen = Screen::Models;
    type_text(&mut m, "nonexistent");
    assert_eq!(app::handle_key(&mut m, KeyEvent::new(KeyCode::Enter, KeyModifiers::NONE)), app::Flow::Continue);
}
