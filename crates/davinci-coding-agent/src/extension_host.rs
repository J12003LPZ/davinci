...
pub enum ExtensionEvent {
    ...
    Input {
        ...
        images: Option<Vec<davinci_ai::MessageContent>>,
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct InputEventResult {
    pub action: String,
    pub text: String,
    pub images: Vec<davinci_ai::MessageContent>,
}
...
impl ExtensionHost {
    ...
    pub fn load_with_cwd(agent_dir: &Path, names: &[String], cwd: &Path) -> Self {
        ...
        if node_available() {
            for manifest in &host.manifests {
                ...
                let themes: Vec<Value> = davinci_tui::builtin_themes()
                    .into_iter()
                    .map(|theme| {
                        serde_json::json!({
                            "name": theme.name,
    ...
    pub fn filter_models(&self, models: &mut Vec<davinci_ai::Model>) {
        models.retain(|model| {
            !self
    ...
    ) -> Result<usize, davinci_agent::ToolError> {
        self.native
            .lock()
    ...
    pub fn execute_native_tool(
        ...
        if crate::native_extensions::package_intelligence::TOOL_NAMES.contains(&name) {
            let package = self
                .native
                .lock()
                .unwrap_or_else(|e| e.into_inner())
                        ...
                        match value.get("action").and_then(Value::as_str) {
                            ...
                            Some("transform") => {
                                ...
                                if let Some(next_images) = value.get("images") {
                                    if let Ok(parsed) =
                                        serde_json::from_value::<Vec<davinci_ai::MessageContent>>(
                                            next_images.clone(),
                                        )
                                    {
    ...
    pub fn execute_named_tool(&self, name: &str, cwd: &Path) -> Option<Result<String, String>> {
        // Event notifications do not carry the model-supplied arguments.
        // Extension tools are executed exactly once through
        // execute_js_or_manifest_tool_with_context; running them here with {}
        // duplicates side effects and corrupts argument-dependent commands.
        let _ = (name, cwd);
        None
    }
                ...
                if tool.name == name {
                    if let Some(command) = &tool.command {
                        return execute_command_tool(command, args, cwd, tool.timeout_ms)
                            ...
                            .map_err(davinci_agent::ToolError::Failed);
                    }
                }
            }
        }
        Err(davinci_agent::ToolError::Unknown(name.to_string()))
    }
