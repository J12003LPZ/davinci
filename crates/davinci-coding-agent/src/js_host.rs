...
fn runner_path() -> Result<PathBuf, String> {
    ...
    PATH.get_or_init(|| {
        ...
        let needs_private_write = current.as_deref() != Some(RUNNER_JS.as_bytes());
        #[cfg(unix)]
        let needs_private_write = needs_private_write || {
...
pub fn run_js_refresh_models(
    ...
    allow_network: bool,
    force: bool,
) -> Result<Vec<davinci_ai::Model>, String> {
    let result = run_persistent_js_extension(
        module,
        "refreshModels",
        &serde_json::json!({
    ...
    let models = result
        ...
        .unwrap_or(Value::Null);
    Ok(davinci_ai::models_from_provider_config(
        provider,
