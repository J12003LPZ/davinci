...
pub fn refresh_model_catalogs(
    ...
    for provider in builtin_provider_ids() {
        if let Some(json) = davinci_ai::builtin_catalog_json(provider) {
...
fn refresh_provider(
    ...
    if let Ok(raw) = std::env::var("PI_CATALOG_REFRESH_REPLY") {
        ...
        if let Some(payload) = value.get(provider).cloned().or_else(|| {
            ...
            store.providers.insert(
                ...
                ModelsStoreEntry {
                    models: models.clone(),
                    checked_at: Some(davinci_ai::now_ms()),
                    last_modified: Some(davinci_ai::now_ms()),
                    etag: None,
                },
            );
            return Ok(models);
    ...
    let checked_at = davinci_ai::now_ms();
    if status == 304 {
