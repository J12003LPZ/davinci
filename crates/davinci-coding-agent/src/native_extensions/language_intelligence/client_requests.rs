...
impl ClientRequestState {
    ...
    pub fn handle_request(&self, method: &str, params: &Value) -> std::result::Result<Value, (i64, &'static str)> {
        match method {
            "workspace/configuration" => self.configuration_response(params).map_err(|_| (-32602, "Invalid configuration request")),
            ...
            "window/workDoneProgress/create" => {
                let token = params.get("token").cloned().ok_or((-32602, "Missing progress token"))?;
            ...
            "workspace/applyEdit" => Ok(json!({"applied":false,"failureReason":"DaVinci Language Intelligence is read-only"})),
    ...
    fn configuration_response(&self, params: &Value) -> Result<Value> {
        let items = params
            ...
            .ok_or_else(|| IntelligenceError::new("protocol_error", "workspace/configuration items are missing"))?;
        if items.len() > 64 {
            return Err(IntelligenceError::new("protocol_error", "workspace/configuration exceeds the bounded item count"));
        }
    ...
    fn register(&self, params: &Value) -> std::result::Result<Value, (i64, &'static str)> {
        let registrations = params.get("registrations").and_then(Value::as_array).ok_or((-32602, "Invalid registrations"))?;
        ...
        for registration in registrations {
            let id = registration.get("id").and_then(Value::as_str).ok_or((-32602, "Registration id missing"))?;
            let method = registration.get("method").and_then(Value::as_str).ok_or((-32602, "Registration method missing"))?;
            ...
            let selectors = registration
                ...
                .and_then(Value::as_array)
                .map(|items| {
                    items.iter().filter_map(|item| item.get("language").and_then(Value::as_str).map(str::to_string)).collect()
                })
                .unwrap_or_default();
            state.insert(id.to_string(), Registration { method: method.into(), selectors });
        }
        Ok(Value::Null)
    }
    ...
    fn unregister(&self, params: &Value) -> std::result::Result<Value, (i64, &'static str)> {
        ...
        let mut state = self.registrations.lock().unwrap_or_else(|e| e.into_inner());
        for item in items {
            let id = item.get("id").and_then(Value::as_str).ok_or((-32602, "Unregistration id missing"))?;
            state.remove(id);
        }
        Ok(Value::Null)
    }
...
fn truncate_json_strings(value: &mut Value, max: usize) {
    match value {
        ...
        Value::Array(items) => items.iter_mut().for_each(|item| truncate_json_strings(item, max)),
        Value::Object(map) => map.values_mut().for_each(|item| truncate_json_strings(item, max)),
        _ => {}
    }
}
