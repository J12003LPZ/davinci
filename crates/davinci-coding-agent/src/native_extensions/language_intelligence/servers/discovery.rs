...
pub(super) fn invocation_for_path(path: &Path, args: Vec<String>, search_path: &OsStr) -> Result<ServerInvocation> {
    if !path.is_absolute() {
        return Err(IntelligenceError::new("server_not_installed", "Language-server candidates must resolve to absolute installed paths"));
    }
    ...
    if matches!(extension.as_str(), "cmd" | "bat" | "ps1" | "sh") {
        return Err(IntelligenceError::new("server_not_installed", "Shell launchers are not eligible language-server commands"));
    }
    if matches!(extension.as_str(), "js" | "mjs" | "cjs") {
        let node = path_program(search_path, "node").ok_or_else(|| IntelligenceError::new("server_not_installed", "A verified Node executable is required for this installed language server"))?;
        let script = path.canonicalize().map_err(|_| IntelligenceError::new("server_not_installed", "Language-server script is unavailable"))?;
        let mut node_args = vec![script.to_string_lossy().into_owned()];
        node_args.extend(args);
        let mut invocation = ServerInvocation::new(node, node_args).map_err(|_| IntelligenceError::new("server_not_installed", "Language-server runtime is unavailable"))?;
        invocation.executable_fingerprint = fingerprint_with_files(&invocation, &[script]);
        return Ok(invocation);
    }
    ServerInvocation::new(path.to_path_buf(), args).map_err(|_| IntelligenceError::new("server_not_installed", "Language-server executable is unavailable"))
}

pub(super) fn named(search_path: &OsStr, name: &str, args: Vec<String>) -> Option<ServerInvocation> {
    let path = path_program(search_path, name)?;
    invocation_for_path(&path, args, search_path).ok()
}

pub(super) fn env_absolute(name: &str, args: Vec<String>, search_path: &OsStr) -> Option<ServerInvocation> {
    let path = PathBuf::from(std::env::var_os(name)?);
    path.is_absolute().then(|| invocation_for_path(&path, args, search_path)).and_then(Result::ok)
}
