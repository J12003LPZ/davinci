...
pub(super) fn discover(
    ...
    if !profile.enabled {
        return Err(IntelligenceError::new("disabled", "Rust language intelligence is disabled"));
    }
    let invocation = if let Some(server) = &profile.server {
        discovery::explicit(server, search_path)?
    } else if let Some(invocation) = discovery::env_absolute("DAVINCI_RUST_ANALYZER", Vec::new(), search_path) {
        invocation
    } else {
        discovery::named(search_path, "rust-analyzer", Vec::new()).ok_or_else(|| IntelligenceError::new("server_not_installed", "Install rust-analyzer or configure languageIntelligence.rust.server"))?
    };

    for value in [&profile.toolchain_dir, &profile.sysroot, &profile.sysroot_src].into_iter().flatten() {
        if !value.is_absolute() || !value.is_dir() {
            return Err(IntelligenceError::new("invalid_settings", "Configured Rust toolchain/sysroot paths must be existing absolute directories"));
        }
    }
    if let Some(src) = &profile.sysroot_src {
        if !src.join("core/src/lib.rs").is_file() {
            return Err(IntelligenceError::new("invalid_settings", "rust.sysrootSrc must contain core/src/lib.rs"));
        }
    }

    let fingerprint = settings.profile_fingerprint(profile);
    let target_dir = std::env::temp_dir().join("davinci-language-intelligence").join(&fingerprint[..16.min(fingerprint.len())]);
    let mut cargo = json!({
    ...
    if let Some(target) = &profile.target { cargo["target"] = json!(target); }
    if !profile.features.is_empty() { cargo["features"] = json!(profile.features); }
    ...
    env.insert("CARGO_TARGET_DIR".into(), target_dir.to_string_lossy().into_owned());

    let mut limitations = vec!["proc_macros_disabled".into(), "build_scripts_disabled".into(), "cargo_checks_disabled".into()];
    if profile.sysroot_src.is_none() { limitations.push("rust_src_not_installed".into()); }
