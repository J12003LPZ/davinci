...
pub fn tool_specs() -> Vec<AgentTool> {
    ...
    [
        ...
        ("process_status", "Inspect state, lifetime identity, exit status and environment policy of an owned process. Does not start a process.", json!({"id":id}), vec!["id"]),
        ...
        ("process_write", "Write literal UTF-8 text to an owned active process's stdin (16 KiB maximum). A timeout can mean partial delivery; inspect output before retrying.", json!({"id":id,"text":{"type":"string","maxLength":16384}}), vec!["id","text"]),
        ("process_stop", "Release this owner's process lease. Final release requests descendant cleanup; inspect status until exited. Repeated release is safe; other owners retain their leases.", json!({"id":id}), vec!["id"]),
