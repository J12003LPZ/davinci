...
use davinci_ai::stream::{complete, StreamOptions};
use davinci_ai::types::{ContentBlock, Message, Role};
use thiserror::Error;
...
pub struct AgentConfig {
    ...
    pub max_turns: u32,
    pub fixture: Option<davinci_ai::stream::FixtureResponse>,
    pub permission: Box<dyn PermissionPolicy>,
    pub transport: Option<davinci_ai::Transport>,
            ...
            Ok(assistant) => {
                let text = assistant
                    ...
                    .filter_map(|b| match b {
                        davinci_ai::ContentBlock::Text { text } => Some(text.as_str()),
                        _ => None,
                    })
                    .collect::<Vec<_>>()
                    .join("");
                ...
                for block in &assistant.content {
                    if let davinci_ai::ContentBlock::ToolCall {
                        tool_call_id,
                        tool_name,
                        input,
                    } = block
...
mod tests {
    ...
    use davinci_ai::events::{AssistantMessage, AssistantMessageEvent, StopReason};

    #[test]
    fn fixture_turn_emits_agent_events() {
        let fixture = davinci_ai::stream::FixtureResponse {
            events: vec![
                ...
                AssistantMessageEvent::Done {
                    ...
                    message: AssistantMessage {
                        ...
                        usage: Some(davinci_ai::Usage {
                            input: 1,
                            output: 1,
                            total_tokens: 2,
                            ..davinci_ai::Usage::default()
                        }),
                        error_message: None,
                        timestamp: 1,
                    },
