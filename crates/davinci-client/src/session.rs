...
impl SessionClient {
                ...
                dispatch: Box::new(|_| {
                    (
                        ServerMessage::HelloError {
                            error: ProtocolError {
                                code: davinci_protocol::ProtocolErrorCode::InternalError,
                                message:
    ...
    pub fn list_sessions(&self) -> Result<Vec<davinci_protocol::SessionMetadata>, ClientError> {
...
mod tests {
    ...
    fn snapshot(id: &str, attached: bool, revision: u64, text: &str) -> SessionSnapshot {
        SessionSnapshot {
            ...
            transcript: if text.is_empty() {
                ...
                vec![davinci_protocol::TranscriptItem::User {
                    id: "u1".into(),
                    content: vec![davinci_protocol::TextOrImage::Text { text: text.into() }],
                    timestamp: 1,
                }]
            },
    ...
    fn create_prompt_detach_and_reconnect() {
        ...
        assert!(prompted.transcript.iter().any(|item| matches!(
            item,
            davinci_protocol::TranscriptItem::User { content, .. }
                if content.iter().any(|part| matches!(part, davinci_protocol::TextOrImage::Text { text } if text == "hello"))
        )));
