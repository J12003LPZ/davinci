...
use davinci_session::{now_ms, JsonlSession, SessionEntry, SessionError, SessionSummary};
...
impl SqliteSessionStore {
    ...
    pub fn open_repo_session(&self, id: &str) -> Result<davinci_session::Session, SessionError> {
        ...
        let mut session = davinci_session::Session::with_metadata(id, created_at as u64, parent);
        let mut items = Vec::new();
        {
            ...
            for row in rows {
                ...
                let entry: davinci_session::SessionEntry =
                    serde_json::from_str(&payload).map_err(|err| {
                        SessionError::storage(format!("Unable to decode entry: {err}"))
                    })?;
                items.push(davinci_session::LogItem::Entry {
                    seq: seq as u64,
                    entry,
                });
            }
        }
        {
            ...
            for row in rows {
                ...
                let record: davinci_session::LaneRecord =
                    serde_json::from_str(&payload).map_err(|err| {
                        SessionError::storage(format!("Unable to decode record: {err}"))
                    })?;
                items.push(davinci_session::LogItem::Record {
                    seq: seq as u64,
                    record,
                });
            }
        }
        {
            ...
            for row in rows {
                ...
                items.push(davinci_session::LogItem::Lane {
                    seq: seq as u64,
                    lane,
                    leaf_id,
                });
        ...
        }
        {
            ...
            for row in rows {
                ...
                if kind == "name" {
                    items.push(davinci_session::LogItem::FactName {
                        seq: seq as u64,
                        name: value,
                    });
                } else {
                    items.push(davinci_session::LogItem::FactLabel {
                        seq: seq as u64,
        ...
        items.sort_by_key(davinci_session::LogItem::seq);
        for item in items {
        ...
        for item in session.get_log(&davinci_session::LogOptions::default())? {
            if let davinci_session::LogItem::Lane { lane, .. } = item {
