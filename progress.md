...
- **Session overlay extras**: `/resume` uses SessionSelector with **ctrl+p path**, **ctrl+s sort** (Threaded / Recent / Fuzzy), **ctrl+r rename** (empty name clears).
...
- **Session selector extras**: path/sort/rename are ported; TS still has Tab scope (current folder vs all), named-only filter (`ctrl+n`), delete (`ctrl+d` / ctrl+backspace), and `re:` / `"phrase"` search.
- **Slash leftovers**: `/import`, `/share`, and `/changelog` are listed in the builtin catalog but still parse as unknown. `/reload` reloads skills/templates/context, not keybindings/extensions/themes.
