...
**Self-improving learning system** (`crates/davinci-coding-agent/src/native_extensions/learning/`, design in `docs/superpowers/specs/2026-09-03-davinci-self-improving-learning-design.md`, plan in `docs/superpowers/plans/2026-09-03-davinci-self-improving-learning-plan.md`, docs in `docs/learning.md`): Settled turns hook (`complete_prompt_with_host`) dispatches a background model review in interactive/RPC sessions: `reviewer.rs::execute_live_review` runs this binary as a `-p --no-session --no-tools` child on the session's model (one at a time, `minReviewIntervalMs` apart, never cancelled by a new turn; print mode and graph workers do not review; `PI_LEARNING_REVIEW_FIXTURE` answers synchronously for tests). Finished reviews apply at the next turn or `/learning-*` command, with no approval step by default, and active learned skills matching the prompt are injected beside vector memory (`learned_skill_block`). The reviewer extracts deterministic verification evidence (`evidence.rs`) by analyzing shell commands (`bash`, `powershell`, `execute_command`), `graph_run` outcomes, tool exit codes, tool errors, permission denials, and user acceptance/correction signals:
...
- **Procedural skills** (`SkillCreate`, `SkillPatch`, `SkillSupportFile`): saved as `SKILL.md` under `<agent_dir>/learning/projects/<repo-key>/skills/<name>/` (project; Davinci-owned so no project trust is needed, legacy in-repo stores with recorded skills keep working) or `~/.pi/agent/skills/<name>/` (global) with structured YAML frontmatter. Verified workflows auto-promote to active after deterministic verification (`commands_ran > 0` and zero failures) or 2 verified usages without failures.
...
**Plugins** (`crates/davinci-coding-agent/src/plugins/`, spec `docs/superpowers/specs/2026-09-26-plugin-marketplace-design.md`, guide `docs/plugins.md`; no TS counterpart): Claude Code / Codex format plugins (skills, commands, agents, hooks, MCP servers). `davinci plugin …` and `/plugin …` install from marketplaces into `<agent_dir>/plugins/cache/` or adopt Claude Code / Codex installs in place (`installed.json` `origin`). Plugin resources are the lowest layer: user and project definitions with the same name win. Plugin hooks run only while `hooksApproved` equals the digest of the hook definitions plus referenced files; they never run in graph workers, and `DAVINCI_PLUGINS=off` loads nothing. Tests point `CLAUDE_CONFIG_DIR` / `CODEX_HOME` at temp dirs through `plugins::external::tests::FakeHomes`. Bare `/plugin` opens the manager sheet (`Screen::Extensions`, `views/extensions.rs`; rows and actions in `plugins/manager.rs`): tabs for plugins, skills and MCP servers, with update/enable/approve/delete keys. Every action re-reads current state before acting, skills are moved to `<agent_dir>/trash/skills/` rather than deleted, and MCP edits touch only the `mcp.json` that defines the server.
...
Its current conversation visual contract uses `docs/ui/claude-code-reference/` as the ground truth, with `docs/ui/design.md` documenting DaVinci-specific differences. Enter runs highlighted slash commands, `?` opens the shortcuts panel from an empty composer, and a leading `!` uses shell mode.

The historical HTML mockups and Elixir reference under `docs/ui/` document earlier layouts; they do not override the current conversation design. Current built-in command discovery is defined only by `builtin_slash_commands()` in `slash.rs`; native-extension discovery is defined by `command_specs()` in `native_extensions/mod.rs`. Notable interactive surfaces include `/model`, `/settings`, `/thinking <off|minimal|low|medium|high|xhigh|max>`, `/login`, `/hotkeys`, `/resume`, `/tree`, `/compact`, `/export`, `/graph`, `/setup` (workspace setup: vector memory, `.gitignore`, trust via `/setup trust`, plugin hooks, language servers; `native_extensions/setup.rs`), `/memory-status`, `/governor-status`, `/security-scan`, `/reload`, `/mcp`, `/plugin`, `/permissions`, `/plan`, and `/act`. `/llama`, `/trust`, `/changelog`, `/scoped-models`, and the separate `/sec-status`, `/sec-report`, `/sec-abort`, and `/sec-resume` lifecycle commands are not public slash commands. Security lifecycle operations remain internal to `/security-scan`. Shared sheet framing lives in `views/sheet.rs`. Keep graph, governor, vector-memory, security, and permission surfaces wired to real runtime data. `davinci --davinci --screen <id>` opens fixture screens for visual inspection; it is interactive and blocks.

**Startup responsiveness** (`davinci_interactive.rs`, `davinci_sources.rs`, `startup.rs`): the initial workspace scan uses the existing `WorkspaceDresser` worker. Apply completed results between turns and rebuild the command palette corpus when session data arrives. Package update and tmux checks run in the background; configuration, migration, and trust warnings remain in the opening block. Preserve typed input and transcript indices when background results arrive. Davinci does not query the upstream Pi release endpoint or show Pi version/changelog update notices. Do not reintroduce those notices when updating parity code. `DAVINCI_STARTUP_TRACE=<file>` appends the milliseconds reached by each startup stage, ending at `shell: first frame`. The model runtime snapshot (catalog, extension providers, auth availability) is cached by `load_model_runtime` while its input files, arguments and environment are unchanged, and `/reload` clears it. Measure time to the first usable composer, accounting for terminal-harness startup overhead (a pywinpty launch adds about 3 s by itself); offline timings do not establish startup speed with every user extension or MCP configuration.
