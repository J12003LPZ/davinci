...
Generated continuation was published as `90676ae`; the remote branch matches that commit and draft PR #29 was updated. This is checkpoint evidence, not final acceptance.

## Saved pipeline continuation

Saved pipelines retain their original and writer mutation baselines, native verification/security results and source input identities. Explicit resume retries unfinished nodes while preserving completed workers and failure-branch outcomes. Source changes invalidate final checks/reviews without replaying writers. Missing mutation details at the artifact/checkpoint boundary are reconstructed from the retained baseline before another writer starts. Saved artifact bindings now read validated artifacts from the run directory and report missing inputs instead of silently omitting them.

Regressions reproduced verification repair remaining blocked, missing bound research/plan content, stale source approval, failure branches being rejected as unmet success dependencies, saved definition versions above one being refused, negative reviews being treated as completion, source changing during verification, and a missing writer mutation companion surviving resume. These cases are covered by stage-by-stage reload tests and failure injection. Checkpoint validation now checks the saved compiled definition, task contracts, native check receipts and original writer baselines.

Windows graph suite passed 398 tests; Linux passed 399, with five existing native fixtures ignored each. The Linux RPC regression passed. A follow-up regression reproduced a crash between a negative review artifact and interpretation of its verdict incorrectly completing the run. Saved review restoration now interprets that verdict before dispatch. It also rejects contradictory approvals containing major or blocking issues. All four saved-continuation tests passed on both platforms after that correction. Host library/test clippy with warnings denied passed on Windows and Linux; formatting and diff whitespace passed. Linux evidence: `linux-saved-continuation.log` and `linux-saved-continuation-final.log` in the external evidence directory. Author review only; no subagents.

This remains checkpoint evidence. Complete attempt history, persistent worker sessions and orphan reconciliation remain open, along with legacy migration and the full UI/native acceptance matrix. Next: bind worker attempts to private durable conversations and retain their outcomes without replaying completed mutations.

Saved continuation was published as `bad08c3`; the remote branch matches, and draft PR #29 was updated.

## Durable worker attempt records

The controller now records each worker attempt before dispatch and saves its terminal outcome before retry or stage advancement. The authoritative run checkpoint retains attempt identity, input fingerprint, timing, exit code, PID, usage and timeout flags. Per-attempt artifact and effect-report archives preserve earlier evidence when the worker's current output is replaced. Progress usage is checkpointed while the worker runs. Attempt records and companion publication share the checkpoint lock so another worker cannot publish history referencing an artifact that has not yet been written.

New regressions first reproduced dispatch without an attempt record and acceptance of duplicate attempt history. Coverage includes two failed attempts followed by an on-disk resume to attempt three, unchanged prior outcomes, retained effect reports, progress spending, corrupt history refusal and record-write failures before dispatch and before retry. This retains attempt evidence; it does not yet reopen a private worker conversation or reconcile an orphaned worker.

Windows graph suite passed 401 tests and Linux passed 402, with five existing native fixtures ignored on each. Host library/test clippy with warnings denied passed on Windows and Linux after the publication-lock correction. Formatting and diff whitespace passed. Linux evidence: `linux-attempt-history.log` in the external evidence directory. Private worker sessions, fail-closed conversation persistence, orphan ownership, partial-check recovery, legacy compatibility and the full UI/native gates remain required. Author review only; no subagents.

Attempt history was published as `7a32ffe`; the remote branch matched and draft PR #29 was updated.

## Conversation persistence failure boundaries

Session writes now flush durable storage before advancing in-memory state. Failed writes retain the previous cursor/metadata and latch a recovery requirement until explicit reopen. Header replacement uses a unique staged file and atomic publication. Agent execution checks the latch before provider requests and tool dispatch, and after assistant/tool-result persistence. Tool results preserve their call ID, tool name and error flag. Host JSON output and exit status expose conversation persistence failures even when an earlier assistant message or valid artifact exists. Graph attempts stop with a reconciliation requirement instead of automatically retrying after lost history.

Failure injection first reproduced advanced in-memory cursors after failed writes, lost tool-result identity, provider/tool execution after a lost prompt, hidden host failures, and a second worker dispatch after lost conversation history. Regression coverage now includes all these cases, subsequent reopen, failure at prompt/assistant/result boundaries, durable controller refusal on resume, and artifact publication followed by a conversation write failure.

With `PI_OFFLINE=1`, `cargo test --offline --locked -p davinci-session` passed 41 tests on each platform; `cargo test --offline --locked -p davinci-agent --lib` passed 989 on Windows and 971 on Linux. The graph library selector passed 402 on Windows and 403 on Linux, with five existing native fixtures ignored each. After the final worker result helper/test, its focused suite passed 17 with two existing native fixtures ignored on each platform. The host `session_persistence_failure_reaches_host_reply_and_json_event` regression passed on both. Agent/session and host library/test clippy passed with warnings denied on both; all-workspace formatting and diff whitespace passed. Linux evidence: `linux-session-persistence-final.log` in the external evidence directory. Review was by the implementation author; no subagents.

This is an intermediate checkpoint. Worker sessions are still nonpersistent at launch; task-bound private storage, restart reconciliation, legacy recovery, partial-check reuse, full UI reference comparisons and exact-final-revision native/full-suite gates remain required. Next: connect persistent worker conversations to attempt identity and exclusive ownership without replaying completed mutations.

Conversation persistence boundaries were published as `e539c35`; the remote branch matched and draft PR #29 was updated.

## Worker conversation runtime ownership

Loading a parent-bound worker conversation now preserves its run, worker and parent identity instead of creating an independent root task coordinator. A durable identity record binds the conversation to its source and workspace. Cold reload validates that binding and every runtime event; ordinary root sessions cannot adopt worker history. Workers cannot switch to another conversation. An OS-held writer lock excludes competing opens and releases on process death. Candidate runtime observers are isolated so a failed tool-ledger validation releases ownership without changing the caller.

Regressions first reproduced lost parent authority, unrestricted session switching, competing writers and a leaked writer after failed activation. Tests cover retained messages/runtime sequence, shared parent task authority, foreign lineage refusal, live reload without duplicate observers, process-kill release, and recovery after repairing a corrupt tool ledger. A host test verifies two offline turns retain parent authority and do not create a child task journal.

Windows agent library tests passed 991; Linux passed 973 before the final two focused regressions. On the final source, `PI_OFFLINE=1 cargo test --offline --locked -p davinci-agent --lib worker_session_` passed all four tests on each platform, and `cargo test --offline --locked -p davinci-coding-agent --bin davinci host_worker_session_preserves_parent_authority_across_turns` passed on each. Agent library/test clippy passed with warnings denied on the final source on both platforms; host library/test clippy passed before the candidate-observer isolation correction. Formatting and diff whitespace passed. Linux evidence: `linux-worker-session-runtime.log`. Review was by the implementation author; no subagents.

This is the runtime foundation, not graph worker recovery. Graph launch still uses nonpersistent sessions. Private task-bound session storage, attempt lineage/fork policy, orphan reconciliation, runtime-log failure handling, legacy and partial-check recovery, full reference comparisons and final native/full suites remain open. Next: connect graph launch to private persistent sessions with validated parent authority and attempt identity.
