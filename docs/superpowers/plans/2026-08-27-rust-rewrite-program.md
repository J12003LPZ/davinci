...
Date: 2026-08-27
Status: active
TypeScript source of truth: `vendor/pi` at `853a80d26c90a14c1886f0ebb8ffaae133ca2185` (`earendil-works/pi` main)

## Objective

Port the pi monorepo crate-by-crate from TypeScript to Rust while **TypeScript remains the shipped, authoritative product** until the Phase 8 cutover gate. Rust must prove behavioral parity with differential fixtures and the TypeScript conformance suites, then stay green on `cargo test`, `cargo fmt --check`, and `cargo clippy -- -D warnings`.

## Authority

- TypeScript under `vendor/pi/packages/*` is the executable specification.
- Rust under `crates/*` is the port-in-progress.
- When behavior disagrees, change Rust, not TypeScript, unless a TypeScript bug is independently confirmed.
- Do not delete or replace the TypeScript runtime in this program. npm packages remain the product path until the cutover gate.
...
| 3 | `packages/protocol`, `packages/client`, `packages/server` | `pi-protocol`, `pi-client`, `pi-server` | CBOR/framing vectors + in-memory loopback |
...
| 7 | `packages/coding-agent` | `pi-coding-agent` | CLI print/RPC/session + built-in tools |
| 8 | cross-package | `pi-parity` | differential fixtures + quality gates |
...
9. `pi` CLI supports `--help`, `--version`, print mode, session list, and the four built-in tools.
...
12. TypeScript under `vendor/pi` is still present and documented as authoritative. The default product install is unchanged.

Cutover (default binary becomes Rust; TypeScript moves to maintenance) is **out of scope** for this program. Phase 8 is shadow-complete, not cutover.
...
See `2026-08-27-rust-rewrite-phase-1.md` through `2026-08-27-rust-rewrite-phase-8.md`.
